...
def split_pyg_dataset(dataset):
    import numpy as np
    from sklearn.model_selection import train_test_split

    train_size, val_size, test_size = cfg.dataset.split
    labels = np.array([graph.y.item() for graph in dataset])
    train_idx, test_idx = train_test_split(np.arange(len(dataset)), train_size=train_size, random_state=cfg.seed, stratify=labels)
    val_idx, test_idx = train_test_split(test_idx, test_size=test_size, random_state=cfg.seed, stratify=labels[test_idx])

    set_dataset_attr(dataset, 'train_graph_index', torch.tensor(train_idx), train_size)
    set_dataset_attr(dataset, 'val_graph_index', torch.tensor(val_idx), val_size)
...
@register_loader('custom_create_loader')
def create_loader():
    """
